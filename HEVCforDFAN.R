load('~/dfan.RData')
mssng=subset(dfan$filename,!grepl('RPDNC',dfan$filename))
pb=winProgressBar(min=1,max=len(mssng))
...
bds=data.frame(fname=normalizePath(fns,winslash = '/'),errorC=NA)
...
if(nrow(bdx3))
  bdx3$errorC='AVC'
bdx=rbind(bdx1,bdx2,bdx3)
save(bdx,file='~/BDX.RData')
