  ...
  if(grepl('Volume',volz[1])&!grepl('RPDN',volz[1]))
  {
    ...
    if(!file.exists(sfname)){
      ...
      if(length(extras)>0){
        ...
        if(length(ttle)>1){
          for(kk in 1:(length(ttle)-1)){ # remove comments, title:
            lnn=dff[ttle][kk]
            if(lnn>0){
              for(i in 1:lnn){
...
emsg='OK'
while(TRUE){
liner <- dlgInput(paste("Enter pno search Criteria \n",emsg),default=dflt)$res;
if (!length(liner)) { # The user clicked the 'cancel' button
...
if(length(liner)>0)
  if(nchar(liner)>0)
  {
    ...
    fns=NULL
    if(!is.na(pnoln[1])){
      ttls=unlist(regexpr('Comment|Title|Sub Title|File Path|Ingredients',pnoln))
      ttls[ttls<0]=500
    ...
    if(length(fns)>0){
      ...
      load('headfoot.RData')
      writeLines(as.character(c(header,paste('<media src="',fns,'"/>'),footer),sep=''),'fns.wpl')
